...
- `PlayerHeroTemplate = Monster_22_Skeleton King`?  
- `PetCandidatePool = "Pet Demo/Assets/Fantazia Animated 2D Monsters/Prefabs/*.prefab" - { Monster_22_Skeleton King }`?  
...
2. **??????????**  
   - ??????????????????? 1 ????????  
   - ??????? `Monster_22_Skeleton King`?? ?2.5 ????  
   - ????????????`sceneExistingHero -> serializedHeroPrefab -> fallbackCapsule`?  
   - `serializedHeroPrefab` ??????????????????????? `Monster_22_Skeleton King.prefab`?  
...
4. **???? Spine ?????`HeroRoot`?Demo?**  
   ...
   - **???**??????? `Monster_22_Skeleton King` ?? Skeleton ??? **`Idle`?`Walk`**??????????????????????????????????
...
- `CampInteractionResolver.DispatchFocus` ?? `FarmField` ?? ? `BeginFarmFieldFlow(FarmFieldComponent fc)`?
  ...
  2. ????`Update`?????? FarmField ???? `IsInRange` ?**?**`MoveIdle == true` ??????????????????? `SpineAttackPlayer.PlayAttackOnce()` + `FarmManager.ApplyAttack(fieldId, heroEntityId, save.inventory)`?
  3. ???????????? `_farmAttackTimer`?? `_farmAttackTimer >= crop.attackIntervalSec` ???????? `PlayAttackOnce()` + `ApplyAttack(...)`?
...
- `CampInteractionResolver.DispatchFocus` ?? `GatherPoint` ?? ? `BeginGatherPointFlow(GatherPointComponent gp)`?
  ...
  2. ????`Update`?????? GatherPoint ???? `IsInRange` ?**?**`MoveIdle == true` ??????????????????? `SpineAttackPlayer.PlayAttackOnce()` + `GatherManager.ApplyAttack(pointId, heroEntityId, save.inventory)`?
  3. ???????????? `_gatherAttackTimer`?? `_gatherAttackTimer >= config.attackIntervalSec` ???????? `PlayAttackOnce()` + `ApplyAttack(...)`?
